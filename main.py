...
pref = cursor.execute(f"SELECT prefix FROM info").fetchone()

pref = pref[0]

@app.on_message(filters.command("prefix", prefixes=pref) & filters.me)

def type(_, msg):

	che = msg.text.split()[1]	cursor.execute(f"UPDATE info SET prefix = '{che}' ")

	connect.commit()

	msg.edit(f"Новый префикс успешно установлен!\nНовый префикс: {che}")

@app.on_message(filters.command("spam", prefixes=pref) & filters.me)
...
@app.on_message(filters.command("очистка", prefixes=pref))
...
@app.on_message(filters.command("check", prefixes=pref))
...
@app.on_message(filters.command("gen", prefixes=pref))
...
@app.on_message(filters.command("пинг", prefixes=pref))
...
@app.on_message(filters.command("timer", prefixes=pref) & filters.me)
...
@app.on_message(filters.command("go", prefixes=pref) & filters.me)
...
@app.on_message(filters.command("type", prefixes=pref) & filters.me)
